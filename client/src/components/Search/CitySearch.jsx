    ...
    <div ref={dropdownRef} className="relative w-full max-w-md z-[2000]">
      <div className="relative">
        <input
          ...
          className="w-full bg-slate-800/80 hover:bg-slate-800 text-slate-100 border border-slate-700/80 focus:border-indigo-500 rounded-xl px-5 py-3.5 pr-12 text-sm shadow-xl transition-all duration-300 outline-none placeholder-slate-400 backdrop-blur-md focus:ring-4 focus:ring-indigo-500/10"
          onFocus={() => {
      ...
      {isOpen && suggestions.length > 0 && (
        <div className="absolute top-full left-0 w-full mt-2 bg-slate-900/95 border border-slate-800 rounded-xl shadow-2xl overflow-hidden max-h-72 overflow-y-auto divide-y divide-slate-800/50 backdrop-blur-md animate-fade-in z-[3000]">
          {suggestions.map((item) => (
            <button
              key={item.id}
              onClick={() => handleSelect(item)}
              className="w-full text-left px-5 py-3.5 hover:bg-indigo-600/10 transition-colors flex items-start gap-3 outline-none"
            >
